...
- 通知栏「截屏」按钮触发 → 截取成功后自动拉回 App 做 OCR
- 点通知正文也可随时回到主界面

相关代码：`PickupCaptureAccessibilityService.kt`、`KeepAliveService.kt`、`AppLauncher.kt`。
...
4. 快递 App 打开取件码页 → 下拉通知点「截屏」（或「拍照」/ 相册）
5. 截屏后会自动回到本 App；也可点通知正文进入
6. 核对取件码 → 连接打印机 →「打印取件码」
7. 「打印编排」可调纸张、方向、字号、对齐与行顺序，保存后生效
